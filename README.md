...
| 页面 | 功能 | 界面特点 |
|------|------|---------|
| **Agents** | Agent 卡片列表、CRUD、实时对话 | 响应式卡片网格，每个卡片显示 Provider/Model，一键 Chat |
| **Groups** | Group 管理、多模式聊天 | 卡片列表显示成员数和聊天模式，一键进入 Group Chat |
| **Tasks** | 三列看板拖拽 | To Do / In Progress / Done，支持分配给 Agent/Group |
| **Providers** | 供应商配置中心 | 内置/自定义供应商卡片，支持 Reset/Discover Models |
